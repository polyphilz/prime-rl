...
| `qorl_anchored_grpo` | policy | `rl` on actions | QORL's anchored group-relative rule: speedups for identical non-default plan fingerprints are first replaced by their group median, clipped log-speedup is soft-thresholded, valid siblings provide a non-negative reference, invalid decisions receive a fixed penalty, and exact default duplicates pay a small protocol cost. Incomplete, errored, or malformed-protocol groups receive zero advantage. |
...
| `qorl_anchored_grpo` | `rl` | QORL-specific anchored credit using clipped log-speedup, a soft threshold, valid sibling references, and explicit invalid/default-duplicate penalties. |
